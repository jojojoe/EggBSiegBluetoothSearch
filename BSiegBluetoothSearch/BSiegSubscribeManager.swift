//
//  BSiegSubscribeManager.swift
//  BSiegBluetoothSearch
//
//  Created by Joe on 2023/4/9.
//
...
class BSiegSubscribeManager {
    ...
    var currentProducts: [BSiegSubscribeManager.IAPProduct] = []
    
}

extension BSiegSubscribeManager {
    
    public func fetchPurchaseInfo(block: @escaping (([BSiegSubscribeManager.IAPProduct]) -> Void)) {
        ...
        SwiftyStoreKit.retrieveProductsInfo(Set(iapList)) { result in
            ...
            let localList = priceList.compactMap { BSiegSubscribeManager.IAPProduct(iapID: $0.productIdentifier, price: $0.price.doubleValue.rounded(digits: 2), priceLocale: $0.priceLocale, localizedPrice: $0.localizedPrice, currencyCode: $0.priceLocale.currencyCode)
            }
    ...
    public func subscribeIapOrder(iapType: BSiegSubscribeManager.IAPType, source: String, completionBlock: ((Bool, String?) -> Void)? = nil) { // inSubscribeBool errorString
...
extension BSiegSubscribeManager {
    ...
    func isPurchased(completion: @escaping (_ purchased: Bool) -> Void) {
        ...
        dispatchGroup.notify(queue: .main) {
            ...
            BSiegSubscribeManager.default.inSubscription = hasValid
            completion(hasValid)
        }
    }
