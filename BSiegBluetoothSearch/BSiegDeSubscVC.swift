...
class BSiegDeSubscVC: UIViewController {
    ...
    @objc func monthBgBtnClick(sender: UIButton) {
        BSiegSubscribeManager.default.currentIapType = .month
        updateIapBtnStatus()
    }
    
    @objc func yearBgBtnClick(sender: UIButton) {
        BSiegSubscribeManager.default.currentIapType = .year
        updateIapBtnStatus()
    }
    
    @objc func theContinueBtnClick(sender: UIButton) {
        BSiegSubscribeManager.default.subscribeIapOrder(iapType: BSiegSubscribeManager.default.currentIapType, source: "shop") {[weak self] subSuccess, errorStr in
...
extension BSiegDeSubscVC: WKNavigationDelegate {
...
extension BSiegDeSubscVC {
    
    func updatePrice(productsm: [BSiegSubscribeManager.IAPProduct]?) {
        if let products = productsm {
            let product0 = products[0]
            let product1 = products[1]
            currentSymbol = product0.priceLocale.currencySymbol ?? "$"
            
            if product0.iapID == BSiegSubscribeManager.IAPType.month.rawValue {
    ...
    func fetchPriceLabels() {
        ...
        if BSiegSubscribeManager.default.currentProducts.count == BSiegSubscribeManager.default.iapTypeList.count {
            updatePrice(productsm: BSiegSubscribeManager.default.currentProducts)
        } else {
            updatePrice(productsm: nil)
            BSiegSubscribeManager.default.fetchPurchaseInfo {[weak self] productList in
                guard let `self` = self else {return}
                DispatchQueue.main.async {
                    if productList.count == BSiegSubscribeManager.default.iapTypeList.count {
    ...
    func updateIapBtnStatus() {
        if BSiegSubscribeManager.default.currentIapType == .year {
            yearBgBtn.layer.borderColor = UIColor(hexString: "#3971FF")!.cgColor
            monthBgBtn.layer.borderColor = UIColor(hexString: "#D2D7E5")!.cgColor
            monthSelectImgV.image = UIImage(named: "storeselect")
            yearSelectImgV.image = UIImage(named: "storeselect_s")
        } else if BSiegSubscribeManager.default.currentIapType == .month {
            monthBgBtn.layer.borderColor = UIColor(hexString: "#3971FF")!.cgColor
