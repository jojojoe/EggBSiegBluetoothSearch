...
class PZSwipedCollectionViewCell: UICollectionViewCell,UIGestureRecognizerDelegate {
    ...
    @objc private func pzPanAction(panGestureRecognizer: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            //TODO: fix left move
            centerPoint.x = min( self.frame.width / 2, max(self.snapShotView!.center.x + translationPoint.x, self.frame.width/2 - self.revealView!.frame.width + 20))
    ...
    func showRevealView(withAnimated isAnimated:Bool) {
        UIView.animate(withDuration: isAnimated ? 0.1 : 0 ,
                       ...
                       options: .curveEaseOut,
                       animations: {
                        self.snapShotView?.center = CGPoint(x: self.frame.width / 2 - self.revealView!.frame.width + 20, y: self.snapShotView!.center.y)
        }, completion: { (finished: Bool) in
